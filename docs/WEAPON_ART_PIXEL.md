...
## 当たりの絵のずれ・続き（2026-09-24）
ユーザー報告「ヒットエフェクトがズレる問題がまだ発生している」。前回（当たりの絵を相手に付ける）で直らなかった分：
- **振りの絵（斬撃・槌の火花・槍の先）が振った瞬間の場所に残っていた。** 走りながら殴ると、0.3秒の間に主人公が約1マス進み、
  斬撃と先端の火花が後ろに置き去りになって「敵と違う所」に見えた → `W.fx` の `swing` に `ent`（主人公・仲間）を持たせ、
  `drawFeelSwing` で毎フレーム本人の位置に付ける。
- **当たりの白い火花（`feelSpray`）が当たった瞬間の場所に残っていた。** ノックバックで相手が動くと火花だけ元の場所 → 火花に相手を持たせ、相手が動いた分だけ一緒に動かす。
- **跳ねている敵（`hopLift`）には、当たりの絵が絵より下に出ていた** → 当たりの絵も跳ねた高さだけ上げる。
- `arttest` の `swingFollows`。

## 灰の大蛙の叩きつけ・薙ぎ払いの作り直し（2026-09-24）
ユーザー報告「叩き付けのエフェクトが上手く反映されていなさそう」「薙ぎ払いが舌で薙ぎ払っているように見えない。口元から舌を。薙ぎ払い部分をピンクにしても」。
- 叩きつけ：体の下に描いていた灰の柱・ひびが大蛙の絵に隠れ、残るのは薄い網の煙だけだった → 体の縁から外だけで見せる。
  太い衝撃の輪（つぶれた楕円、網を掛けずに塗り切る）が一気に広がる／体の縁から外へ太いひび（消えるときは先から短くなる）／
  床を這う灰の壁／飛び散る石くず／1コマの閃光。本編は `wide:e.r`（体の半径）を渡し、画面を揺らす（`feelKick(3.2,.22)`）。
- 薙ぎ払い：舌を**口から**出す（`feelBossMouth(e,dir)`：絵の口は中心から右へ0.30・下へ0.14（絵の大きさ比）、向きは `_artFace`）。
  伸ばす（0.07秒）→ 扇の端から端まで振る（0.2秒、先がしなって遅れる）→ 引っ込める（0.1秒）。舌は太く（縁取り・つや・先の膨らみ）、振った跡はピンクの帯。
- 通常攻撃の舌（`bt_jab`）も口から。
