...
- **第二形態（礫の狩人）は岩を投げる形**（2026-09-24 ユーザー指定「石の要素が多い姿になるので岩を飛ばすものに」）：
  溜めで床から岩が浮いて前に集まり大岩になる（`rockChargeAir`）／貫き＝大岩を投げる（`boulderAir`、回りながら飛び、小岩が後を追い、終わりで砕ける）／
  散弾＝足元の岩盤を砕いて岩を全方位へ／**飛ぶ岩はすべて先の尖った「岩の弾丸」`rockBullet()`**（ユーザー指定。後ろは丸く先が尖り、芯に稜線、先端に光。落石も弧の向きに先を向ける）／波動＝輪の先で岩の棘が突き出す（`rockSpike`）／落石＝岩を弧で投げ込む（`bv_rock` に `casterX/Y`）。
- **礫の岩はマット**（ユーザー指定、参考画像あり）：パレット `rk0〜rk5`（明るい縁・明るい面・中・暗・割れ目／影。輪郭には使わない）。面を平らに塗り分け、
  白や青白い照り返しは入れない。塊は `rock()`、弾は `rockBullet()`、波動の棘は `bigSpike()`（段のある岩の柱。少し傾き、先が欠けることもある）。
- **2026-09-24 岩はスプライトに置き換え**（ユーザー指定「参考画像に合わせて」→ 静止画で検討 → 採用）：
  参考画像の描き方を学ばせて作った `ROCK_SPR`（4色：黒＝輪郭と割れ目・暗 #3b3e43・中 #5a5a5b・明 #7b7c7c、パレット `rs1〜rs4`）。
  弾丸は大・小×16方向（光はどの向きでも左上）、棘は高さ 18・24・30×3種（地面から上の行だけ見せて生える／沈む）、塊は大・中・小×4種。
  `rock()` `rockBullet()` `bigSpike()` はこのスプライトを描く。作り方の道具は `tools/rock-sprites/`（README に手順）。
- （以下は置き換え前の手描きの岩の記録）
- **輪郭線は引かない**（ユーザー指定「アウトラインがうるさい。削って陰影で立体感を」）：明るさ5段 `rk0〜rk4` の段で形を出す。
  面の向きの明暗＋縁の陰影（光の側の縁は一段明るく、影の側の縁は一〜二段暗く、`rimShade`）＋下側・根元の接地の暗がり。
- **波動は地面から大きな岩の棘が突き出す**：輪が通った所（輪の内側）に床の決まった位置で生え、残ってから崩れて沈む。**棘は床の層（ground）に描く**＝主人公とボスは必ず手前。
...
| `isFloor` | `(sx,sy)=>bool`（画面座標）。渡すと、床でない所には描かない（ヴェラの波動の棘） |
...
## ヴェラの散弾・波動の不具合修正（2026-09-24）
- 散弾の矢・岩の下に、旧い弾の絵（`drawFeelMagic` の色の尾）が重なって描かれていた → `artId` のある弾は描かない（index.html）。
- 散弾（第一形態）の発動の光が「14本の線を決まった向き」に引いていて、実際の矢（9本・激昂14本、向きは揺らぎあり）とずれていた → 光と輪だけにした。
- 光の矢を網掛けで薄めていたので、斜めだと点線にちぎれて見えた → 全部塗り切り、矢じりを「く」の字に。
- 散弾（第二形態）の岩片を網掛けで薄めて消していたので、最後に点の格子（四角い点々）が残った → 薄めずに小さくなって消える。土煙も明るく薄く。
  絵の余白（`ext`）も広げた（端で四角く切れていた）。
- 波動の棘が壁・奈落の上にも生えていた → `isFloor` で床の上だけ。棘の影と割れ目の線も網掛けをやめた（点線に見えた）。上の余白 `sky` を広げた（上端の棘の先が切れていた）。
